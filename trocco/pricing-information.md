...
- 30日トライアルプラン（詳しくは、[30日トライアルプラン・フリープランに関する詳細情報](/docs/free-plan-guide)を参照ください）
- フリープラン（詳しくは、[30日トライアルプラン・フリープランに関する詳細情報](/docs/free-plan-guide)を参照ください）
- Starterプラン
- Essentialプラン
- Advancedプラン
- Professionalプラン
...
|コネクタ種別 \各プラン|30日トライアル|フリー|Starter|Essential|Advanced|Professional|
|---|---|---|---|---|---|---|
...
TROCCOでは、アカウント内で同時に実行できるジョブの数に上限が設けられています。
ジョブを実行すると該当のジョブはいったん`QUEUED`（実行待ち）状態となり、その後ジョブの実行が開始されます。

本ページでは、同時実行数の上限値、カウント対象となるジョブ、上限に達した場合の挙動、およびダッシュボード上での確認方法について説明します。

#### 契約プランごとの上限値

ご契約のプランによって、ジョブの同時実行数上限の値は異なります。

|30日トライアル|フリー|Starter|Essential|Advanced|Professional|
|---|---|---|---|---|---|
|5|5|10|20|50|100\*|
...
ジョブの同時実行数上限は、2024年04月に料金プランが変更されたことを受けて一部仕様が変更されています。
旧プランの仕様は、以下の通りです。
...
同時実行数の上限に達している状態で新たにジョブを起動した場合、該当のジョブは実行されずエラーとなるのではなく、`QUEUED`（実行待ち）状態のまま待機します。
先行するジョブが終了して同時実行数の枠が空いた時点で、待機していたジョブの実行が自動的に開始されます。ユーザー側での再実行操作は必要ありません。

:::(Info) (スケジュール集中による遅延)

[スケジュール機能](/docs/schedule-settings)で定期実行するジョブが同一時刻に集中すると、同時実行数上限が上限に達し、後続のジョブ実行が`QUEUED`となり遅延する場合があります。
...
TROCCOのダッシュボード画面では、アカウントの同時実行数の状況を**同時実行数サマリー**で確認できます。
時刻の表示はすべて日本時間（UTC+9）です。

- **現在実行中のジョブ数**：画面表示時点での`SETTING UP`または`EXECUTING`状態にあるジョブの数を確認できます。
- **過去利用状況**：直近3ヶ月における、各月のジョブ同時実行数の最大値が表示されます。過去の傾向をつかむのに便利です。
- **ジョブの最大実行数推移**：ジョブの最大実行数推移を毎時・日次・月次で確認できます。過去利用状況でつかんだ傾向の掘り下げに便利です。

:::(Info) (グラフの活用例)

- メンテナンス作業を計画する際に、ジョブ実行数が少ない時間帯・曜日を把握する
- 同時実行数上限への到達が頻発していないかを確認し、必要に応じてスケジュール時刻の分散やプランのアップグレードを検討する

:::

:::(Warning) (グラフデータの更新頻度)

推移グラフのデータは定期的に集計されるバッチ処理によって更新されます。
そのため、反映までに数分から数十分程度の遅延が発生する場合があります。
...
\*2 30日トライアルプランおよびフリープランについては追加課金は発生せず、代わりにジョブの実行が制限されます。
...
|30日トライアル|フリー|Starter|Essential|Advanced|Professional|
|---|---|---|---|---|---|
|10時間/月|2時間/月|30時間/月|250時間/月|600時間/月|（お問い合わせください）|

#### 処理時間が処理時間枠を超過した場合

処理時間が処理時間枠を超過した場合の振る舞いは、プランによって異なります。

- 30日トライアルプランまたはフリープランをお使いの場合
