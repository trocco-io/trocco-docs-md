...
|2026/06/02|取得対象アクションの追加<br>および[audit_log_action_list.csv](/docs/about-audit-log#アクション)の更新|[TROCCO API](/docs/trocco-api)を利用してdbtジョブ設定を操作した際のアクションを追加しました。<ul><li>`DbtJobConfigurationCreatedFromAPI`,dbtジョブ設定の作成（APIを利用）</li><li>`DbtJobConfigurationUpdatedFromAPI`,dbtジョブ設定の更新（APIを利用）</li><li>`DbtJobConfigurationDeletedFromAPI`,dbtジョブ設定の削除（APIを利用）</li></ul>|
