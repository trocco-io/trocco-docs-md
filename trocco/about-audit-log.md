...
|`ProcessingTimeNotification`|処理時間通知|
...
|2026/04/21|取得対象のカテゴリー・アクションの追加<br>および[audit_log_action_list.csv](/docs/about-audit-log#アクション)の更新|[処理時間機能](/docs/processing-time)の処理時間通知に関する監査ログが取得可能になりました。<br>上記に伴い、取得対象のカテゴリーとアクションを追加しました。<ul><li>カテゴリー<ul><li>`ProcessingTimeNotification`,処理時間通知</li></ul></li><li>アクション<ul><li>`ProcessingTimeNotificationByTotalCountCreated`,処理時間通知を作成</li><li>`ProcessingTimeNotificationByTotalCountUpdated`,処理時間通知を更新</li><li>`ProcessingTimeNotificationByTotalCountDeleted`,処理時間通知を削除</li></ul></li></ul>|
|2026/05/25|取得対象アクションの追加<br>および[audit_log_action_list.csv](/docs/about-audit-log#アクション)の更新|[TROCCO API](/docs/trocco-api)を利用してdbt Gitリポジトリ設定を操作した際のアクションを追加しました。<ul><li>`DbtGitRepositoryCreatedFromAPI`,dbt Gitリポジトリ設定の作成（APIを利用）</li><li>`DbtGitRepositoryUpdatedFromAPI`,dbt Gitリポジトリ設定の更新（APIを利用）</li><li>`DbtGitRepositoryDeletedFromAPI`,dbt Gitリポジトリ設定の削除（APIを利用）</li></ul>|
|2026/06/11|取得対象アクションの追加<br>および[audit_log_action_list.csv](/docs/about-audit-log#アクション)の更新|[TROCCO API](/docs/trocco-api)を利用してdbtジョブ設定を操作した際のアクションを追加しました。<ul><li>`DbtJobConfigurationCreatedFromAPI`,dbtジョブ設定の作成（APIを利用）</li><li>`DbtJobConfigurationUpdatedFromAPI`,dbtジョブ設定の更新（APIを利用）</li><li>`DbtJobConfigurationDeletedFromAPI`,dbtジョブ設定の削除（APIを利用）</li></ul>|
