...
起動時に必要な`registory_name`、`repository_name`は、TROCCOのSelf-Hosted Runner Cluster詳細画面にてご確認ください。

Runner起動時に指定する環境変数は以下となります。
...
以下は、docker runを利用してRunnerを起動する際のサンプルコマンドです。

```bash
# 1. 最新のイメージを取得する
docker pull public.ecr.aws/<registry_name>/<repository_name>:latest

# 2. 最新のイメージでRunnerを起動する
docker run \
...
以下は、Docker Composeを利用してRunnerを起動する際の`compose.yaml`のサンプルです。

```yaml
services:
  trocco-self-hosted-runner:
    image: public.ecr.aws/<registry_name>/<repository_name>:latest
    environment:
      TROCCO_REGISTRATION_TOKEN: "<YOUR REGISTRATION TOKEN>"
```

また、以下はRunnerを起動する際のサンプルコマンドです。
...
TROCCOは、Runner用のコンテナイメージを継続的に更新しています。
機能改善やセキュリティ対応を反映するため、定期的に最新のイメージへ更新してください。

更新は、以下の流れで行います。

1. 最新のコンテナイメージを取得する（`docker pull`など）。
2. 起動中のRunnerを新しいイメージで起動し直す。

イメージの取得元（`registry_name`・`repository_name`）は、Self-Hosted Runner Cluster詳細画面でご確認ください。
以降の手順では、`public.ecr.aws/<registry_name>/<repository_name>`をイメージとして記載します。

:::(Info) (更新中のジョブへの影響)

ジョブの実行中（[ACTIVE](/docs/self-hosted-runner#runnerのステータス)）のRunnerを停止しても、そのRunnerは割り当て済みのジョブが終了するまで動作します（Graceful Shutdown）。
そのため、更新作業によって実行中のジョブが中断されることはありません。
詳しくは、[Runnerの停止](/docs/self-hosted-runner#runnerの停止)を参照ください。
:::

:::(Info) (更新前のRunnerの扱い)

更新前の古いRunnerは、停止後に[OFFLINE](/docs/self-hosted-runner#runnerのステータス)となり、72時間後に自動的に削除されます。
すぐに画面上から消したい場合は、手動で削除してください。
詳しくは、[Runnerの削除](/docs/self-hosted-runner#runnerの削除)を参照ください。
:::

### docker runを利用している場合

最新のイメージを取得したうえで、Runnerを起動し直します。
起動時のパラメーターについては、[Runnerの起動](/docs/self-hosted-runner#2-runnerの起動)を参照ください。

```bash
# 1. 最新のイメージを取得する
docker pull public.ecr.aws/<registry_name>/<repository_name>:latest

# 2. 起動中のコンテナを停止する（<container_name>は対象コンテナの名前またはID）
docker stop <container_name>

# 3. 最新のイメージでRunnerを起動し直す
docker run \
  -e TROCCO_REGISTRATION_TOKEN="<YOUR REGISTRATION TOKEN>" \
  --rm \
  public.ecr.aws/<registry_name>/<repository_name>:latest
...
`docker compose`を利用している場合は、イメージの再取得とコンテナの再作成をまとめて実行できます。
`compose.yaml`を配置したディレクトリで、以下のコマンドを実行します。

```bash
# 1. 最新のイメージを取得する
docker compose pull

# 2. 最新のイメージでコンテナを再作成する
docker compose up -d --force-recreate
```

`--force-recreate`を指定することで、イメージに変更がない場合でもコンテナが再作成されます。

:::(Info) (compose.yamlでイメージのバージョンを固定している場合)

`compose.yaml`でイメージのバージョンを固定している場合は、まず`compose.yaml`の`image`で指定しているバージョンを変更してください。
