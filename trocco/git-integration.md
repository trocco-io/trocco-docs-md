...
現在、TROCCOでは[Terraform Provider for TROCCO](/docs/terraform-provider-for-trocco)の提供を開始しています。
...
:::(Warning) (署名コミット必須のリポジトリルールには対応していません)

TROCCOのGitリポジトリ連携では、コミットへの署名に対応していません。
そのため、連携先リポジトリで以下のいずれかが有効になっている場合、TROCCOからのpushは`Commits must have verified signatures.`というエラーで失敗します。
