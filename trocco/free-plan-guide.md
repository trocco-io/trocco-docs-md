...
- フリープランへの移行時、フリープランでは利用できない機能に関連するスケジュールは自動的に無効化されます。
  - フリープランで利用不可となるコネクタを使用している転送設定のスケジュール
  - 増分更新 / SCD Type 2 / 品質チェック設定を使用しているデータマート定義のスケジュール
  - ワークフロー定義のすべてのスケジュール

:::(Info) (設定の再利用)
有料プランへアップグレードすると、保持されていた設定を再びご利用いただけます。
自動で無効化されたスケジュールは、再度有効化を行う必要があります。
:::
