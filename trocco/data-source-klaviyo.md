...
|項目名|必須|デフォルト値|説明|
|---|---|---|---|
|`取得開始日時（datetime 以上）`|-|-|取得対象期間の開始日時を入力できます。<br>フォーマットは`YYYY-MM-DDTHH:MM:SS+HH:MM`（例：`2025-01-01T00:00:00+09:00`）で入力してください。|
|`取得終了日時（datetime 未満）`|-|-|取得対象期間の終了日時を入力できます。<br>フォーマットは`YYYY-MM-DDTHH:MM:SS+HH:MM`（例：`2025-12-31T23:59:59+09:00`）で入力してください。|
